...
const MockObject: React.FC<{
    ...
    const [objectProps, setObjectProps] = useState(defaultValue || '');
    // 数量范围状态
        ...
        <div style={{ margin: '16px 0' }}>
            ...
            {mock === 'count' && (
                <div>
                    ...
                    />
                    <span style={{ marginLeft: 8 }}>当前对象共 {Object.keys(parseProperties()).length} 个属性</span>
                </div>
            )}
            ...
            <div style={{ marginTop: 16, padding: 12, background: '#f5f5f5' }}>
                <h4>实时预览：</h4>
                <pre>{JSON.stringify(parseProperties(), null, 2)}</pre>
            </div>
        </div>
    );
};
