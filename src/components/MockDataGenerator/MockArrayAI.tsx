...
const MockArray: React.FC<{
    ...
    const [arrayStr, setArrayStr] = useState(defaultValue || '');
    ...
    const generatePreview = useCallback(() => {
        switch (mock) {
            ...
            case 'min-max':
                ...
                return `${name}|${min}-${max}: ${JSON.stringify(
                    arrayStr
                        .split(',')
                        .map((s) => s.trim())
                        .filter(Boolean)
            ...
            case 'count':
                return `${name}|${count}: ${JSON.stringify(
                    arrayStr
                        .split(',')
                        .map((s) => s.trim())
                        .filter(Boolean)
            ...
            {mock === 'min-max' && (
                <>
                    <Alert
                        ...
                        style={{ marginBottom: 16 }}
                    />
                    <Input value={arrayStr} onChange={handleArrayChange} placeholder="输入基础数组项，用逗号分隔" />
